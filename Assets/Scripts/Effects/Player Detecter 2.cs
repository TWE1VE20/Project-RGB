...
public class PlayerDetecter2 : SFX_ControlledObject
{
    ...
    public override void Setup()
    {
        ...
        weaponGo.transform.position = ViewPoint.position;
        weaponGo.transform.rotation = ViewPoint.rotation;
    ...
    private void ActivateLasers(GameObject target)
    {
        ...
        if (raycast)
        {
            for (int i = 0; i < LasersCount; i++)
            {
                ...
                var randomCircle = GetPosOnCircle(target.transform.position, raycastHit.point.y, angle, LaserRadiusOverLifeTime.Evaluate(0));
